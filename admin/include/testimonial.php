...
<div class="page-header">
  <div class="container">
    <h3 class="page-title float-left">Add Testimonial</h3>
          ...
          <article class="" data-animation="">
              <section class="section-comment">
					<div id="comment-reply-form">
						<form method="post" id="testimonial_form" name="testimonial_form" class="form-full-width">
						<input  id="txthidden" name="txthidden" type="hidden"  value="">
						<div class="col-md-6">
							
							<img  id="txtimg"  />
							<input type="file" id="upload_img" name="upload_img" style="border:0;">
							<label style="">Do You Want To Display:</label>
						...
						<div class="col-md-6">
							<textarea id="testimonial_desc" name="testimonial_desc" required placeholder="Description"></textarea>
			   ...
			   <table class="table table-responsive"> 
						...
						<tbody id="show_test">
							<?php include('show_testimonial.php');?>
						</tbody>
				</table>
           </article>
        </section>
...
function postData()
{
	...
	var testimonial_desc=$('#testimonial_desc').val();
	var is_display=$('#is_display').val();
	var file = new FormData();
        file.append('upload_img', $('#upload_img')[0].files[0]);
		 
		 file.append('testimonial_desc', testimonial_desc);
	...
	if(btnValue=="Save")
	{
		...
		$.ajax({
		 ...
		 type: 'POST',
		 async : false,
		 url: '<?php echo DIR_INCLUDES.'testimonial_post.php';?>',
		 cache: false,
		 contentType: false,
		 processData:false,
		 data: file,
...
//Remove Category
function removetestCat(test_id)
{
	var test_id=test_id;
...
<script type="text/javascript">
//Edit Category
function edittest(editid,desc,img,isd)
{
		
	
	$('#testimonial_desc').val(desc);
	$('#txtimg').attr({src:'<?php echo DIR_TESTIMONIAL ?>'+img,height:'150px',width:'150px'});
	if(isd==0)
	{
		$('.chk1').attr('checked',true);
	}
	...
	var editid=editid;
	$.post("index.php?url=TEST", {"editid": editid});
}
</script>
</html>
